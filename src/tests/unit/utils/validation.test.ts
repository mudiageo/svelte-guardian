...
describe('Validation Utils', () => {
	it('should validate password according to schema Prisma adapter', async () => {
		...
		const isValidPassword = validatePassword( 'rddbknbkjbjkjkv', passwordPolicy);
...
describe('Password Validation', () => {
  ...
  describe('Default Options', () => {
    ...
    it('should fail if missing uppercase', () => {
      expect(validatePassword('weakpassword123!').message).toContain('Password must contain at least one uppercase letter');
    });

    it('should fail if missing lowercase', () => {
      expect(validatePassword('ALLUPPERCASE123!').message).toContain('Password must contain at least one lowercase letter');
    });

    it('should fail if missing number', () => {
      expect(validatePassword('CompletelyAlphabetic!').message).toContain('Password must contain at least one number');
    });

    it('should fail if missing special character', () => {
      expect(validatePassword('NoSpecialCharsPass123').message).toContain(`Password must contain at least one special character from: ${DEFAULT_SPECIAL_CHARS}`);
    });
  });
  ...
  describe('Length Validation', () => {
    ...
    it('should fail if too short', () => {
      ...
      expect(validatePassword('Short1!', options).message).toContain(`Password must be at least ${options.minLength} characters long`);
    });
  ...
  describe('Specific Character Requirements', () => {
    it('uppercase requirement with number', () => {
      ...
      expect(validatePassword('Oneuppercasepass123!', options).message).toContain(`Password must contain at least ${options.requireUppercase} uppercase letters`);
    });
    ...
    it('lowercase requirement with number', () => {
      ...
      expect(validatePassword('aLLUPPERCASE123!', options).message).toContain(`Password must contain at least ${options.requireLowercase} lowercase letters`);
    });
    ...
    it('number requirement with number', () => {
      ...
      expect(validatePassword('OneNumberPass1!', options).message).toContain(`Password must contain at least ${options.requireNumbers} numbers`);
    });
    ...
    it('special character requirement with number', () => {
      ...
      // Should pass
      expect(validatePassword('TwoSpecialChars!!Pass123', options)).toHaveProperty('success', true);
      console.log(validatePassword('OneSpecialChar!Pass123', options))
      // Should fail with only one special character
      expect(validatePassword('OneSpecialChar!Pass123', options).message).toContain(`Password must contain at least ${options.requireSpecialChars} special characters from: ${options.specialChars}`);
    });
  });
  ...
  describe('Custom Special Characters', () => {
    it('should validate against custom special characters', () => {
      ...
      // Should fail
      expect(validatePassword('InvalidPass123!', options).message).toContain(`Password must contain at least one special character from: ${options.specialChars}`);
    });
  });
  ...
  describe('Complex password policy', () => {
    ...
    it('should fail if any requirement is not met', () => {
      ...
      expect(validatePassword('oneupper2low2Nums!Pass', options).message).toContain(/uppercase/);

      // Fails due to insufficient lowercase
      expect(validatePassword('TWOUPPER2LOW2NUMS!PASS', options).message).toContain(/lowercase/);

      // Fails due to insufficient numbers
      expect(validatePassword('TwoUPlow1Num!Pass', options).message).toContain(/numbers/);

      // Fails due to insufficient special characters
      expect(validatePassword('TwoUPlow2Nums1Pass', options).message).toContain(/special character/);
    });
  });
...
//   });
  ...
  describe('Performance and Boundary Conditions', () => {
    it('should handle very long passwords within max length', () => {
      const longPassword = 'A'.repeat(64) + '1!';
...
//     const isValidPassword = validatePassword('short');

//            const errorMessages = isValidPassword.message;
//         expect(errorMessages[0]).toMatch(/at least/i);
